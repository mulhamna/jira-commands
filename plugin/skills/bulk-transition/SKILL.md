...
Bulk transition multiple Jira issues using the `jira` CLI.
...
4. Run: `jira issue bulk-transition -p <PROJECT> -q '<JQL>' -t '<STATUS>' [--force]`
5. The CLI will ask for confirmation unless `--force` is used. Inform the user how many issues will be affected.

Examples:
- "transition all To Do issues in PROJ to In Progress" → `jira issue bulk-transition -p PROJ -q 'project = PROJ AND status = "To Do"' -t 'In Progress'`
- "close all done issues in PROJ without asking" → `jira issue bulk-transition -p PROJ -q 'project = PROJ AND status = Done' -t 'Closed' --force`
